package com.example.mockspace.pdf.service;

import com.example.mockspace.pdf.model.*;
...
public class CodingErrorPdfInvoiceCreator {
    ...
    public   void createTnc(List<String> TncList,Boolean lastPage,String imagePath) {
        if(lastPage) {
            ...
            pdfDocument.addEventHandler(PdfDocumentEvent.END_PAGE,
                    new MyFooter(document,TncList,imagePath));
        }
    ...
    public void createProduct(List<Product> productList) {
        ...
        document.add(getDividerTable(fullwidth).setBorder(new SolidBorder(
                Color.convertRgbToCmyk(new DeviceRgb(255,255,255)),1)).setMarginBottom(15f));
    }
    ...
    public void createTableHeader(ProductTableHeader productTableHeader) {
        ...
        threeColTable1.setBackgroundColor(Color.convertRgbToCmyk(new DeviceRgb(0,0,0)),0.7f);

        threeColTable1.addCell(new Cell().add(new Paragraph("Description"))
                .setBold().setFontColor(Color.convertRgbToCmyk(new DeviceRgb(255,255,255)))
                .setBorder(Border.NO_BORDER));
        threeColTable1.addCell(new Cell().add(new Paragraph("Quantity"))
                .setBold().setFontColor(Color.convertRgbToCmyk(new DeviceRgb(255,255,255)))
                .setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
        threeColTable1.addCell(new Cell().add(new Paragraph("Price"))
                .setBold().setFontColor(Color.convertRgbToCmyk(new DeviceRgb(255,255,255)))
                .setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER)).setMarginRight(15f);
    ...
    public void createHeader(HeaderDetails header) {
        ...
        table.addCell(new Cell().add(new Paragraph(header.getInvoiceTitle()))
    ...
    static Table fullwidthDashedBorder(float[] fullwidth)
    {
        ...
        Border dgb=new DashedBorder(Color.convertRgbToCmyk(new DeviceRgb(220,220,220)),0.5f);
    ...
    static Cell getHeaderTextCell(String textValue)
    {
        ...
        return new Cell().add(new Paragraph(textValue))
                .setBold().setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.RIGHT);
    }
    ...
    static Cell getBillingandShippingCell(String textValue)
    {
        ...
        return new Cell().add(new Paragraph(textValue))
                .setFontSize(12f).setBold().setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.LEFT);
    }

    static  Cell getCell10fLeft(String textValue,Boolean isBold){
        Cell myCell=new Cell().add( new Paragraph(textValue))
                .setFontSize(10f).setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.LEFT);
